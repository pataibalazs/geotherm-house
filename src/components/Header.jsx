        ...
        <div className="hidden lg:flex lg:gap-x-10">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="text-lg font-bold text-black hover:text-green-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600 rounded"
            >
              {item.name}
            </Link>
          ))}
              ...
              <div className="space-y-2 py-6">
                {navigation.map((item) => (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="-mx-3 text-lg font-bold block rounded-lg px-3 py-2 text-gray-900 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600"
                  >
                    {item.name}
                  </Link>
                ))}
